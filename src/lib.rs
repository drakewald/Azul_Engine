...
impl GameState {
    ...
    pub fn run_tiling_phase(&mut self) {
        let next_starter_idx = self
            .players
            .iter()
            .position(|p| p.has_first_player_marker)
...
impl PlayerBoard {
    ...
    fn will_complete_horizontal_row(&self, pattern_line_idx: usize) -> bool {
        ...
        let tiles_on_wall_row = self.wall[pattern_line_idx]
            .iter()
    ...
    pub fn is_placement_valid(&self, pattern_line_idx: usize, tile_color: Tile) -> bool {
        ...
        if !line.is_empty() && line[0] != tile_color {
            return false;
        }
        if let Some(col_idx) = WALL_LAYOUT[pattern_line_idx]
            .iter()
            .position(|&t| t == tile_color)
        {
    ...
    pub fn run_tiling_phase(&mut self, discard_pile: &mut Vec<Tile>) -> bool {
        ...
        for row_idx in 0..NUM_ROWS {
            if self.pattern_lines[row_idx].len() == row_idx + 1 {
                ...
                if let Some(col_idx) =
                    WALL_LAYOUT[row_idx].iter().position(|&t| t == tile_color)
                {
                    if self.wall[row_idx][col_idx].is_none() {
                        ...
                        tiles_to_discard[row_idx] =
                            std::mem::take(&mut self.pattern_lines[row_idx]);
        ...
        for mut line in tiles_to_discard {
    ...
    pub fn calculate_end_game_bonuses(&self) -> u32 {
        ...
        for color_to_check in [
            ...
            let count = self
                ...
                .iter()
                .flatten()
                .filter(|&&tile| tile == Some(color_to_check))
                .count();
            if count == 5 {
                bonus_score += 10;
            }
...
impl WasmGame {
    ...
    pub fn new(player_config_js: JsValue) -> Result<WasmGame, JsValue> {
        let player_types_as_u8: Vec<u8> = serde_wasm_bindgen::from_value(player_config_js)
        ...
        if !(2..=4).contains(&num_players) {
            return Err(JsValue::from_str(
                "Invalid number of players. Must be 2, 3, or 4.",
            ));
        }
    ...
    pub fn get_legal_moves(&self) -> Result<JsValue, JsValue> {
        serde_wasm_bindgen::to_value(&self.state.get_legal_moves())
            .map_err(|e| JsValue::from_str(&e.to_string()))
    }

    #[wasm_bindgen(js_name = applyMove)]
    pub fn apply_move(&mut self, move_js: JsValue) -> Result<(), JsValue> {
        let player_move: Move = serde_wasm_bindgen::from_value(move_js)
            .map_err(|e| JsValue::from_str(&e.to_string()))?;
